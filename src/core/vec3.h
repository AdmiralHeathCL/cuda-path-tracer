...
class vec3 {
    public:
        RT_HD vec3() {}
        RT_HD vec3(float e0, float e1, float e2) {
            e[0] = e0;
            e[1] = e1;
            e[2] = e2;
        }
        ...
        RT_HD inline const vec3& operator+() const {
            return *this;
        }
        RT_HD inline vec3 operator-() const {
            return vec3(-e[0], -e[1], -e[2]);
        }

        // read-only
        RT_HD inline float operator[](int i) const {
            return e[i];
        }
        // write
        RT_HD inline float& operator[](int i) {
            return e[i];
        }
        ...
        RT_HD inline float length() const {
            return sqrt(e[0]*e[0] + e[1]*e[1] + e[2]*e[2]);
        }
        RT_HD inline float squared_length() const {
